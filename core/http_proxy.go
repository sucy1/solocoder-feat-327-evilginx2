...
func NewHttpProxy(hostname string, port int, cfg *Config, crt_db *CertDb, db *database.Database, bl *Blacklist, developer bool, tm *TemplateManager) (*HttpProxy, error) {
	p := &HttpProxy{
					...
					if s, ok := p.sessions[ps.SessionId]; ok {
						...
						if err == nil {
							...
							if l.Redirector != "" {
								if !p.isForwarderUrl(req.URL) {
									...
									if _, err := os.Stat(index_found); !os.IsNotExist(err) {
										html, err := p.tm.GetFileContent(index_found)
										if err == nil || (err != nil && html != nil) {
							...
							if _, err := os.Stat(path); !os.IsNotExist(err) {
								fdata, err := p.tm.GetFileContent(path)
								if err == nil || (err != nil && fdata != nil) {
