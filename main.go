...
var hot_reload = flag.Bool("hot-reload", false, "Enable template hot reload (watch for file changes)")
...
func main() {
	...
	tm, err := core.NewTemplateManager(*hot_reload, cfg)
	if err != nil {
		log.Fatal("template manager: %v", err)
		return
	}
	defer tm.Close()

	if *hot_reload {
		tm.AddDirectory(*phishlets_dir)
		tm.AddDirectory(*redirectors_dir)
	}

	hp, _ := core.NewHttpProxy(cfg.GetServerBindIP(), cfg.GetHttpsPort(), cfg, crt_db, db, bl, *developer_mode, tm)
	hp.Start()
